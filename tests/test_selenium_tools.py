...
@patch('selenium_tools.webdriver.Chrome')
@patch('selenium_tools.ChromeService')
def test_initialize_selenium(mock_service, mock_chrome):
    selenium_tools.initialize_selenium()
    mock_service.assert_called_once()
    mock_chrome.assert_called_once()


@patch('selenium_tools.webdriver.Chrome.get')
@patch('selenium_tools.WebDriverWait.until')
def test_open_page_wait(mock_until, mock_get):
    # Simulamos que el driver ya está inicializado
    selenium_tools.driver = MagicMock()
    selenium_tools.driver.get = mock_get  # Asegúrate de que quit es un MagicMock
    # Simulamos la respuesta de BeautifulSoup con un HTML falso
    selenium_tools.driver.page_source = '<html><body><div class="gLFyf"></div></body></html>'

    # Llamamos a la función open_page_wait con la URL de ejemplo y la clase del campo de búsqueda
    soup = selenium_tools.open_page_wait("http://example.com", "gLFyf")
...
@patch('selenium_tools.webdriver.Chrome.quit')  # Parchea el método quit()
def test_close_driver(mock_quit):
    # Simulamos que el driver ya está inicializado
    selenium_tools.driver = MagicMock()
    selenium_tools.driver.quit = mock_quit  # Asegúrate de que quit es un MagicMock
    selenium_tools.driver.closed = False  # Marcamos que no está cerrado

    # Llamamos a la función close_driver
    selenium_tools.close_driver()
