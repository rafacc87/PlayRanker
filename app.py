...
from selenium_tools import close_driver, initialize_selenium, open_page_wait
from utility import get_alias, string_parse_for_url, matcher_game, convert_time, load_config, process_time
...
def metracritic_score(game_title):
    ...
    try:
        ...
        # Score review
        score_review = soup.find('div', class_='c-siteReviewScore')
        if score_review:
        ...
        # Score user
        score_user = soup.find('div', class_='c-siteReviewScore_user')
        if score_user:
            score_user_span = score_user.find('span')
            if score_user_span:
                if score_user_span.text != 'tbd':
                    result_user = float(score_user_span.text.strip())
    ...
    if result_review == 0:
        try:
            metacritic_critic_reviews_url = f"{metacritic_url}critic-reviews/"
            response = requests.get(metacritic_critic_reviews_url, headers=headers)
            ...
            if scores:
                result_review = sum(scores) / len(scores)
                if result_review < 1:
                    result_review = 1
        except Exception as e:
            pass
    ...
    if result_user == 0:
        try:
            metacritic_user_reviews_url = f"{metacritic_url}user-reviews/"
            response = requests.get(metacritic_user_reviews_url, headers=headers)

            soup = open_page_wait(metacritic_user_reviews_url, 'c-siteReviewScore')
            scores_div = soup.find_all('div', class_='c-siteReviewScore')
            ...
            if scores:
                result_user = sum(scores) / len(scores)
                if result_user < 1:
                    result_user = 1
        except Exception as e:
            pass
